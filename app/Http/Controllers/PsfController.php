...
class PsfController extends Controller
{
    ...
    { 
        try{
             $data = $this->info::join('general_acce_details', 'general_acce_details.ams_info_id', 'ams_info.id')
                        ...
                        //   ->whereDate('comp_post_app_stages.job_card_closed', '<=' ,date('d-m-y'))
                          ->get();
    ...
    public function testMail(){
        $subject="Test Mail";
        $data=['body'=>"test Mail"];
        Mail::send('emails.myTestMail',["data"=>$data],function ($message) use ( $subject) {
            $message->to(['ashutosh.rawat@cogenteservices.in'])->bcc(['ashutosh.rawat@cogenteservices.in'])->subject($subject);
            $message->from('accident.care@ashokleyland.com');
        });
    }
}
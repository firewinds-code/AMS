...
//Devendra Developer April 8 2023
...
public function complaintListJobCard(Request $request)
{
          ...
          try
           {
             if(
                $request->chassis_number != null || $request->veh_reg_no != null || $request->customer_name != null || $request->job_card_no != null)
              { 
                ...
                return response()->json(['warning'=>true, 'message'=> 'fill Atleast One Value in  Feild !']);
            }
                catch(\Exception $ex)
                {
    ...
    public function getData(){
        try {
            // $rowData = ams_info::where('flag',0)->orderBy('Plant_Name','ASC')->limit('1')->get()->toArray();
            ...
            foreach($rowData as $row){
                ...
                // $val .= $row['id'].'~~'.$row['Job_card_No'].'~~'.$row['Plant_Name'].'~~'.$row['SAC_Code'].'~~'.$row['Accident_In_charge_Name'].'~~'.$row['Accident_In_charge_contact_Number'].'~~'.$row['Zone'].'~~'.$row['Zone'].'##';
...
// data storing for the question section 
 $questionQuery = [
...
$amsIdExist = DocumentStatus::where('ref_id', $amsid)->doesntExist();
  ...
  $preStage =["ref_id"=>$amsid,
    ...
    // "supplementary_required" => $request->supplementary_required,
  ...
  $preAnalysis =  ["delay_prepare_dealer" => $request->delay_prepare_dealer,
  "ref_id"=>$amsid,
  ...
  $repairAnalysis = ["delay_start" => $request->delay_start,
  "ref_id"=>$amsid,
   ...
   $postAnalysis =  ["delay_final_inspection" => $request->delay_final_inspection,
   "ref_id"=>$amsid,
    ...
    $row = ams_info::select(
        'ams_info.id as amsid', 'ams_info.Vehicle_Registration_Number','ams_info.chassis_number','comp_repair_stages.repair_work_start_dealer','comp_pre_app_stages.initial_estimate_val_dealer',
        'vehicle_details.model','ams_info.Job_Card_open_date','ams_info.SAC_Code','ams_info.Job_card_No','vehicle_details.bs_nonbs',
        'ams_info.Accident_In_charge_Name','ams_info.Accident_In_charge_contact_Number','vehicle_details.date_of_sale','vehicle_details.segment',
        'vehicle_details.customer_email_id','vehicle_details.customer_name', 'vehicle_details.name_of_the_insurance','comp_repair_stages.repair_completion')
        ->leftjoin('comp_pre_app_stages', 'ams_info.id', '=', 'comp_pre_app_stages.ref_id')
        ->leftjoin('comp_repair_stages', 'ams_info.id', '=', 'comp_repair_stages.ref_id')
        ->leftjoin('vehicle_details', 'ams_info.id', '=', 'vehicle_details.ams_info_id')->where('ams_info.id', $request->id)->first();

        $jobcardopen = false;
        $completion = false;
        $workstart = false;

        $mail = ['customer_email' => $row->customer_email_id, 'email_case' => 'job_card_open_date', 'flag' => 1, 'mail_case_date' => $row->Job_Card_open_date];

        if($row->Job_Card_open_date != '')
        {
           $checkMail = MailLog::where('customer_email',  $row->customer_email_id)->where('email_case', 'job_card_open_date')
            ->where('mail_case_date', $row->Job_Card_open_date)->where('flag', 1)->doesntExist();

            if($checkMail)
            {

            try {
                Mail::to($row->customer_email_id)->send(new AshokLeylandMail($row, $jobcardopen = true, $completion, $workstart));
                if(MailLog::where('customer_email',  $row->customer_email_id)->exists())
                {
                    MailLog::where('customer_email', $row->customer_email_id)->update($mail);
                }else{
                    MailLog::create($mail);
                }
               } catch (\Exception $ex) {
                ExceptionLog::create(['type'=>'repair_mail_send', 'exception' => $ex->getMessage(),'job_card_no' => $row->Job_card_No]);
        ...
        if($row->repair_work_start_dealer != '')
        {
           $checkMail = MailLog::where('customer_email',  $row->customer_email_id)->where('email_case', 'repair_work_start_date')
                               ->where('mail_case_date', $row->repair_work_start_dealer)->where('flag', 1)->doesntExist();

            if($checkMail)
            {
                try {
                     Mail::to($row->customer_email_id)->send(new AshokLeylandMail($row, $jobcardopen, $completion, $workstart = true));
                     if(MailLog::where('customer_email',  $row->customer_email_id)->exists())
                     {
                         MailLog::where('customer_email', $row->customer_email_id)->update($mail);
                     }else{
                        ...
                        ExceptionLog::create(['type'=>'repair_work_start_mail_send', 'exception' => $ex->getMessage(),'job_card_no' => $row->Job_card_No]);

                }
            }
        }

        if($row->repair_completion != '')
        {
           $checkMail = MailLog::where('customer_email',  $row->customer_email_id)->where('email_case', 'repair_completion_date')
                               ->where('mail_case_date', $row->repair_completion)->where('flag', 1)->doesntExist();

            if($checkMail)
            {
               try {
                    Mail::to($row->customer_email_id)->send(new AshokLeylandMail($row, $jobcardopen, $completion = true, $workstart));
                    if(MailLog::where('customer_email',  $row->customer_email_id)->exists())
                    {
                        MailLog::where('customer_email', $row->customer_email_id)->update($mail);
                    }else{
                       ...
                       ExceptionLog::create(['type'=>'repair_completion_mail_send', 'exception' => $ex->getMessage(),'job_card_no' => $row->Job_card_No]);
                }
            }
         }
